...
export class DesignBuilder {
  ...
  pin(name: string, dir: BorderPin["dir"], side: Side, index: number): this {
    this.#design.pins.push({ name, dir, side, index });
    return this;
  }

  input(name: string, side: Side, index: number): this {
    return this.pin(name, "in", side, index);
  }

  output(name: string, side: Side, index: number): this {
    return this.pin(name, "out", side, index);
  }

  /** @param sides Any of the letters n, e, s, w */
